...
ролики именно по этой нише в России за последние 6 месяцев, отсортированные
по числу просмотров (свежие вирусные «миллионники» в теме пользователя) —
единственный источник ссылок, без стороннего веб-поиска. Затем ИИ (через
ProxyAPI) собирает подборку из 10 примеров со ссылкой на реальный ролик и
идеей, что из него перенять.
...
Тебе дали список реальных роликов YouTube именно по этой нише (или близкой тематике), опубликованных за последние 6 месяцев, с наибольшим числом просмотров в России — настоящие свежие вирусные ролики-«миллионники», которые уже посмотрели тысячи и миллионы людей. Это единственный источник фактов и ссылок, других данных у тебя нет.
...
Ролики YouTube по нише за последние 6 месяцев с наибольшим числом просмотров в России (единственный источник фактов и ссылок, бери дословно):
{context}
...
async def fetch_youtube_niche_videos(query: str, max_results: int = 30) -> list[dict]:
    ...
    Берём только то, что опубликовано за последние {FRESHNESS_MONTHS} месяцев, чтобы не тащить старьё.
    """
    published_after = (datetime.now(timezone.utc) - timedelta(days=FRESHNESS_MONTHS * 30)).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
    base_params = {
        "part": "snippet",
    ...
    if items:
        return items
    # Если по нише не нашлось коротких — берём вирусные ролики любой длины (но всё ещё свежие).
    items = await _youtube_search_and_detail(base_params)
    if items:
        return items
    # Совсем ничего свежего не нашлось — снимаем ограничение по дате, чтобы не остаться без подборки.
    return await _youtube_search_and_detail({k: v for k, v in base_params.items() if k != "publishedAfter"})
